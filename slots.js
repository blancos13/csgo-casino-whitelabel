...
*/
app.get('/softswiss/games', (req, res) => {
...
app.post('/softswiss/getDemoGame/:provider/:id', async (req, res) => {
  // not connected to any authentication because its not needed, just a demo
  const data = await requestToSoftswiss('/demo', {
    game: `${req.params.provider}:${req.params.id}`,
    urls: {return_url: `${SSVALUES[STAGE].base_url}/slots`}
...
app.post('/softswiss_dev/:action', (req, res) => handleGCPRquest(req, res, req.params.action));
app.post('/softswiss/:action', (req, res) => handleGCPRquest(req, res, req.params.action));
...
/**
 * This will send a request to softswiss backend server
...
async function slotsHandler(r, user, socket) {
  ...
  if(r.command == 'get game') {
    const data = await requestToSoftswiss('/sessions', {
      "game": r.game,
      "currency": "EUR",
...
function getUser(userid, cb) {
  pool.query('SELECT `username`, `avatar`, `userid` FROM `users` WHERE `userid` = ' + pool.escape(userid), function(err1, row1) {
    if(err1) return;
    if(row1.length == 0) return;
