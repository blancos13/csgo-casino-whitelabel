...
<script type="text/javascript">
          ...
          <div class="table-row">
            <div class="table-column text-left">
              ...
              <div class="table-column text-left winn">
                <div>${parseFloat(t.amount_calculated).toFixed(2)}<i class="coins mr-1"></i></div>
              ...
              <div class="table-column text-left winn" id="${uid}" style="text-transform:capitalize">
                <button class="approve" onclick="trxAction('${t.amount}', '${t.userid}', ${t.time}, 'decline')">Approve</button>
  ...
  const updateTrxStatus = (amount, userid, time, status) => {
    ...
    document.getElementById(`${userid}_${time}_${amount}`).style.color = status == 'Done' ? '#dd3a3a' : '#2ec92e';
  }
    ...
    for(let i=0; i<amount - 1; i++) {
      ...
      html += `
        <div class="table-row">
          ...
          <div class="table-column text-left winn">
            <div data-val="${user.classic_profit}" style="color:${user.classic_profit >= 0 ? 'var(--roulette-green)' : 'var(--roulette-red)'}">${parseFloat(user.classic_profit).toFixed(2)}<i class="coins mr-1"></i></div>
            <p>${parseFloat(user.classic_wager).toFixed(2)}</p>
          </div>

          <div class="table-column text-left winn">
            <div data-val="${user.slots_profit}" style="color:${user.slots_profit >= 0 ? 'var(--roulette-green)' : 'var(--roulette-red)'}">${parseFloat(user.slots_profit).toFixed(2)}<i class="coins mr-1"></i></div>
            <p>${parseFloat(user.slots_wager).toFixed(2)}</p>
          </div>

          <div class="table-column text-left winn">
            <div>${parseFloat(user.balance).toFixed(2)}<i class="coins mr-1"></i></div>
          </div>

          <div class="table-column text-left winn">
            <div>${parseFloat(user.deposit_total).toFixed(2)}<i class="coins mr-1"></i></div>
          </div>

          <div class="table-column text-left winn">
            <div>${parseFloat(user.withdraw_total).toFixed(2)}<i class="coins mr-1"></i></div>
          </div>
    ...
    for(let i in trx2) {
      ...
      html += `
        <div class="table-row">
          ...
          <div class="table-column text-left winn">
            <div>${parseFloat(user.amount).toFixed(2)}<i class="coins mr-1"></i></div>
          </div>
...
<div class="admin">
  ...
  <h3 style="margin-top:50px">
    <span>Awaiting Dls withdrawals</span>
  </h3>
