...
switch ($page) {
	...
	case 'admin':
		$rankk = $db->quote($user['rank']);
		if($rankk !== "'1'" && $rankk !== "'100'" && $_SERVER['HTTP_HOST'] !== 'localhost') {
			header('location: /');
			return;
		}
		...
		$balance['total'] = number_format($balance['total'], 5);
		...
		// bets
		$sql_bets = $db->query('SELECT userid, service, amount FROM `users_transactions` WHERE service LIKE "%_bet" OR service LIKE "%_win"');
		$bets = $sql_bets->fetchAll(PDO::FETCH_ASSOC);

		// transactions
		$sql_trx = $db->query('SELECT userid, service, amount, time FROM `users_transactions` WHERE service LIKE "%_deposit" OR service LIKE "%_withdraw"');
		$trx = $sql_trx->fetchAll(PDO::FETCH_ASSOC);
	...
	case 'profile':
		...
		// echo 'SELECT COALESCE(SUM(referral_deposited.amount), 0) AS `amount`, COALESCE(SUM(referral_deposited.commission), 0) AS `commission`, referral_uses.userid FROM `referral_uses` LEFT JOIN `referral_deposited` ON referral_uses.userid = referral_deposited.userid WHERE referral_uses.referral = '.$db->quote($user['userid']).' AND COALESCE(referral_deposited.referral, '.$db->quote($user['userid']).') = '.$db->quote($user['userid']).' GROUP BY referral_uses.userid ORDER BY `amount` DESC LIMIT 50';
		...
		foreach($affiliates_deposited as $key => $value){
			// get more user data
			$userdatamore = $db->query('SELECT avatar, name FROM users WHERE userid = "'.$value['userid'].'"');
			if($userdatamore->rowCount() > 0) {
				$userdatamore2 = $userdatamore->fetch(PDO::FETCH_ASSOC);
	...
	case 'auth':
		switch($paths[1]) {
			case 'login':
				...
				auth_authByLogin(array( 'username' => $username, 'password' => $password ), function($err1, $session = array()){
			...
			case 'register':
				...
				auth_authByRegister(array( 'email' => $email, 'username' => $username, 'password' => $password ), function($err1, $session = array()){
					if($err1) exit(json_encode(array('success' => false, 'error' => $err1)));
				...
				try{
					...
					if (!$openid->mode) {
						...
						if($data_parms['assign']){
							...
							auth_authBySteam(array( 'id' => $steamid, 'name' => $name, 'avatar' => $avatar ), function($err1, $session = array()){
								if($err1) exit($err1);
				...
				try{
					...
					if(!isset($_GET['code'])){
						...
						if($data_parms['assign']){
							...
							auth_authByGoogle(array( 'id' => $id, 'email' => $email, 'name' => $name, 'avatar' => $avatar ), function($err1, $session = array()){
								if($err1) exit($err1);
