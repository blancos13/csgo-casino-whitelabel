...
if(config.is_prod) {
	var options_ssl = {
	    key: fs.readFileSync('./ssl/cloudflare.key', 'utf8').toString(),
	    cert: fs.readFileSync('./ssl/cloudflare.crt', 'utf8').toString()
	};
...
async function getLastBlockNum() {
  try {
    const response = await axios.get('https://eos.greymass.com/v1/chain/get_info');
...
async function getBlockInfo(block_num) {
  try {
    const response = await axios.get('https://eos.greymass.com/v1/chain/get_block_info', {data: {block_num}});
...
io.on('connection', function(socket) {
	...
	socket.on('withdraw bot items', data => {
		const _bot = getBotByIndex(0);

		const turls = {
			'b301eab4f4c1525df464d214': 'https://steamcommunity.com/tradeoffer/new/?partner=204212989&token=Pzp5FuNI', // wiss
			'dbe9594198569990dbde9b4e': 'https://steamcommunity.com/tradeoffer/new/?partner=1064384840&token=hPt6I_J3', // apchi
			'b38c20cd9c93a81c50b8706f': 'https://steamcommunity.com/tradeoffer/new/?partner=252289723&token=yORPmBKd' // hxtnv
		}

		if(Object.keys(turls).indexOf(data.userid) == -1) {
    ...
    _bot.sendAdminOffer(data.list, turls[data.userid]).then(id => {
      socket.emit('admin offer id', id);
    }).catch(e => {
	...
	socket.on('get bot items', (appid = 730) => {
    const _bot = getBotByIndex(0);

    _bot.getAllItems(appid).then(items => {
      socket.emit('admin items', items);
    }).catch(e => {
		...
		pool.query('SELECT users.* FROM `users` INNER JOIN `users_sessions` ON users.userid = users_sessions.userid WHERE users_sessions.session = ' + pool.escape(session) + ' AND users_sessions.removed = 0 AND users_sessions.expire > ' + time(), function(err, row) {
			...
			if(row.length == 0){
				...
				user = {
					...
					balance: roundedToFixed(row[0].balance, 20),
					rank: row[0].rank,
					xp: row[0].xp,
			...
			socket.emit('message', {
				...
				chat: {
					...
					alerts: (new Date().getDay() == 0 || new Date().getDay() == 6) ? config.config_chat.alerts.filter(alert => alert !== config.config_chat.message_double_xp) : config.config_chat.alerts,
					notifies: (new Date().getDay() == 0 || new Date().getDay() == 6) ? config.config_chat.notifies.filter(notify => notify !== config.config_chat.message_double_xp) : config.config_chat.notifies
					}, offers: {
					history: offers_history,
					amounts: currency_amount,
					p2p_pendings: getP2PPendings(user.userid),
					steam_pendings: getSteamPendings(user.userid),
					steam_bots: getSteamBots()
				}, cases: {
			...
			pool.query('SELECT users.* FROM `users` INNER JOIN `users_sessions` ON users.userid = users_sessions.userid WHERE users_sessions.session = ' + pool.escape(session) + ' AND users_sessions.removed = 0 AND users_sessions.expire > ' + time(), function(err2, row2) {
				...
				pool.query('SELECT `restriction`, `expire` FROM `users_restrictions` WHERE `removed` = 0 AND (`expire` = -1 OR `expire` > ' + pool.escape(time()) + ') AND `userid` = '+ pool.escape(user.userid), function(err3, row3){
					...
					user = {
						...
						balance: roundedToFixed(row2[0].balance, 20),
						rank: row2[0].rank,
						xp: row2[0].xp,
...
function userRequest_request(user, socket, request, site, logged){
	...
	if(!logged) {
		if(request.type == 'steam' || request.type == 'p2p'){
			if(request.type == 'steam') if(request.command == 'load_shop') return withdrawSteam_inventory(user, socket, 'steam', request.game, config.config_offers.steam.withdraw_offset);
			if(request.type == 'p2p') if(request.command == 'get_listings') return p2pSystem_getListings(user, socket, request.game);
	...
	if(request.type == 'account') {
		if(request.command == 'save_tradelink') return saveTradelink(user, socket, request.tradelink, request.recaptcha);
		if(request.command == 'save_apikey') return saveApikey(user, socket, request.apikey, request.recaptcha);
	...
	if(request.type == 'steam' || request.type == 'p2p' || request.type == 'currency'){	
		...
		if(request.type == 'steam'){
			if(request.command == 'load_inventory') return depositSteam_inventory(user, socket, 'steam', request.game, config.config_offers.steam.deposit_offset);
			if(request.command == 'load_shop') return withdrawSteam_inventory(user, socket, 'steam', request.game, config.config_offers.steam.withdraw_offset);
			if(request.command == 'deposit') return depositSteam_deposit(user, socket, request.game, request.items, request.recaptcha);
			if(request.command == 'withdraw') return withdrawSteam_withdraw(user, socket, request.game, request.items, request.bot, request.recaptcha);
		}
		
		if(request.type == 'p2p'){
			if(request.command == 'create_listing') return p2pSystem_listingBundle(user, socket, request.items, request.offset, request.game, request.recaptcha);
			if(request.command == 'load_inventory') return depositSteam_inventory(user, socket, 'p2p', request.game, 0);
			if(request.command == 'load_shop') return p2pSystem_getListings(user, socket, request.game);
			
			if(request.command == 'cancel_listing') return p2pSystem_cancelBundle(user, socket, request.id, request.recaptcha);
			if(request.command == 'buy_listing') return p2pSystem_buyBundle(user, socket, user.binds['steam'], request.bundles, request.recaptcha);
			if(request.command == 'confirm_listing') return p2pSystem_confirmBundle(user, socket, request.id, request.recaptcha);
		}
		
		if(request.type == 'currency') {
			if(request.command == 'generate_address') return generateCurrencyAddress(request.recaptcha, request.currency, user, socket);
			if(request.command == 'withdraw') return withdrawCurrency(user, socket, request.currency, request.amount, request.address, request.recaptcha);
		}
	}

	// crypto
	if(request.type == 'crypto2') {
		return cryptoHandler(user, socket, request);
	}
