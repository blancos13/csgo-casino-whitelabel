...
function offers_verifyApikey(user, apikey, callback){
	...
	if(!user.tradelink) return callback(new Error('Firstly, you must set your valid Wallet from settings.'));
...
function depositSteam_confirmDeposit(user, game, items, callback){
	...
	if(!user.tradelink) return callback(new Error('Error: Firstly, you must set your valid Wallet from settings.'));
...
function depositSteam_inventory(user, socket, method, game, offset){
	...
	if(!user.tradelink){
		socket.emit('message', {
			type: 'offers',
			command: 'error',
			error: 'Firstly, you must set your valid Wallet from settings.'
		});
		setUserRequest(user.userid, 'deposit', false, true);
		return;
	}
...
function withdrawSteam_confirmWithdraw(user, game, items, bot, callback){
	...
	if(!user.tradelink) return callback(new Error('Error: Firstly, you must set your Wallet from settings.'));
