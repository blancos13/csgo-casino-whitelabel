...
const hook = new Webhook("https://discord.com/api/webhooks/1073644774864715879/1Cx1gBxKf0Pu9C-S03kfLa6aK-ZnoKdHnmwFoB5yeuT-5VvEY44x5I-WOGJg86y3Cc-L");
const hooks = new Webhook("https://discord.com/api/webhooks/1073644974589100062/PCLz5HE9oumcQdxWSf2HteKXta4pLk1rvQu1tSRHKMBAru60Au9E11o4iqjQo8HnzKrF");
		...
		pool.query('UPDATE `users` SET `balance` = `balance` - ' + amount + ' WHERE `userid` = ' + pool.escape(user.userid), function(err2) {
			...
			pool.query('SELECT COALESCE(SUM(referral_deposited.amount), 0) AS `amount`, referral_uses.referral FROM `referral_uses` LEFT JOIN `referral_deposited` ON referral_uses.referral = referral_deposited.referral WHERE referral_uses.userid = ' + pool.escape(user.userid) + ' GROUP BY referral_uses.referral', function(err3, row3) {
				...
				pool.query('INSERT INTO `crash_bets` SET `userid` = ' + pool.escape(user.userid) + ', `name` = ' + pool.escape(user.name) + ', `avatar` = ' + pool.escape(user.avatar) + ', `xp` = ' + parseInt(user.xp) + ', `amount` = ' + amount + ', `game_id` = ' + parseInt(crashGame.id) + ', `auto_cashout` = ' + roundedToFixed(parseFloat(auto / 100), 2) + ', `time` = ' + pool.escape(time()), function(err4, row4) {
					...
					hooks.send('[CRASH] Bet registed. ' + user.name + ' did bet $' + getFormatAmountString(amount));
					setUserRequest(user.userid, 'crash', false, true);
				});
			});
		});
...
function crashGame_cashout(user, socket) {
	if (crashGame.status !== "counting") {
	  socket.emit("message", {
		type: "error",
		error: "Error: The game has ended!",
	  });
	  setUserRequest(user.userid, "crash", false, true);
	  return;
	}
  
	if (crashGame_settings.point < 1.0) {
	  socket.emit("message", {
		type: "error",
		error: "Error: You can't cash out before 1.00x!",
	  });
	  setUserRequest(user.userid, "crash", false, true);
	  return;
	}
	...
	const winning = getFormatAmount(
	  crashGame_userBets[user.userid]["amount"] * crashGame_settings.point
	);
	...
	pool.query('INSERT INTO `users_transactions` SET `userid` = ' + pool.escape(user.userid) + ', `service` = ' + pool.escape('crash_win') + ', `amount` = ' + pool.escape(winning) + ', `time` = ' + pool.escape(time()));
	
	pool.query('UPDATE `users` SET `balance` = `balance` + ' + pool.escape(winning) + ' WHERE `userid` = ' + pool.escape(user.userid), function(err1){
		if(err1){
			logger.error(err1);
			writeError(err1);
			setUserRequest(user.userid, 'crash', false, true);
		...
		pool.query('UPDATE `crash_bets` SET `cashedout` = 1, `point_cashedout` = ' + pool.escape(roundedToFixed(crashGame_settings.point, 2).toFixed(2)) + ' WHERE `id` = ' + pool.escape(crashGame_userBets[user.userid]['id']));
		...
		logger.debug('[CRASH] Win registered. ' + user.name + ' did win $' + getFormatAmountString(winning) + ' with multiplier x' + roundedToFixed(crashGame_settings.point, 2).toFixed(2));
		hooks.send('[CRASH] Win registered. ' + user.name + ' did win $' + getFormatAmountString(winning) + ' with multiplier x' + roundedToFixed(crashGame_settings.point, 2).toFixed(2));
		setUserRequest(user.userid, 'crash', false, true);
	});	
}
...
function crashGame_generateGame(){
	...
	var crashPointFromHash = function(serverSeed) {
		...
		if (divisible(serverSeed, 100 / parseInt(config.config_games.games.crash.instant_chance, 50))) return 0;
...
function crashGame_checkPoint(){
	...
	if(point >= crashGame.roll){
		...
		pool.query('UPDATE `crash_rolls` SET `ended` = 1 WHERE `id` = ' + parseInt(crashGame.id), function(err1){
			...
			hook.send('[CRASH] Creshed at ' + roundedToFixed(crashGame.roll / 100, 2).toFixed(2) + 'x');
		});
	}
}
		...
		if(crashGame_userBets[bet.user.userid]['cashedout'] == false) {
			...
			if(crashGame_userBets[bet.user.userid]['infinity_cashout'] == false){
				...
				pool.query('UPDATE `users` SET `balance` = `balance` + ' + winning + ' WHERE `userid` = ' + pool.escape(bet.user.userid), function(err2){
					...
					hooks.send('[CRASH] Win registed. ' + bet.user.name + ' did win $' + getFormatAmountString(winning) + ' with multiplier x' + roundedToFixed(parseFloat(winning / crashGame_userBets[bet.user.userid]['amount']), 2).toFixed(2));
